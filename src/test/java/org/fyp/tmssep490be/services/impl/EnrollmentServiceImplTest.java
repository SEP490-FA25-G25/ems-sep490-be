...
class EnrollmentServiceImplTest {
    ...
    void shouldDetectDuplicateEmailsInExcelFile() {
        ...
        List<StudentEnrollmentData> parsedStudents = Arrays.asList(
                createStudentData("Student A", "student@email.com", null),
                createStudentData("Student B", "student@email.com", null) // Duplicate email
        );
    ...
    private List<StudentEnrollmentData> createParsedStudentsData(int count) {
        ...
        for (int i = 0; i < count; i++) {
            students.add(createStudentData("Student " + (i + 1), "student" + i + "@email.com", null));
        }
        return students;
    }

    private StudentEnrollmentData createStudentData(String name, String email, StudentResolutionStatus status) {
        return StudentEnrollmentData.builder()
                // REMOVED: studentCode - auto-generated by system
                .fullName(name)
                .email(email)
                .phone("0901234567")
                .gender(Gender.MALE)
                .dob(LocalDate.of(1995, 1, 1))
                // REMOVED: Skill assessments - handled separately
                .status(status)
                .build();
    }
